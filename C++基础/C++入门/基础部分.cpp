...
b.malloc返回的都是void* ；operator new —>int*
c.new不仅可以做内存开辟，还可以做内存的初始化操作
d.malloc开辟内存失败，是通过返回值和nullptr作比较；而new开辟内存失败是通过抛出bad_alloc类型的异常来判断
e.malloc和free，都是c的库函数，而new和delete都是运算符
3.new有多少种
a.int *p1=new int(20);//抛出异常的new
b.int *p2=new (nothrow) int;//不抛出异常的new
c.const int *p3=new cosnt int(40);//在堆上开辟一个常量空间并初始化为40
d.int *p4=new(某地址) int(50);定位new
