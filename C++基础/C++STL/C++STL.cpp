...
查询：
iterator(连续的insert和erase一定要考虑迭代器失效的问题，对于list来说insert不会引起迭代器失效的问题，因为并没有空间的释放与扩容，earse会引起失效，因为被删除的节点将会被释放)
常用方法：
size()
empty()
...
/*
总结
vector的特点：动态数组，内存是连续的，2倍的方式进行扩容; 默认定义vector<int>vec时，其并没有开辟内存，只有当进行插入操作时，才会开始按照0-1-2-4-8...来扩容；reserve()，仅预留空间，不初始化元素
...
2.底层数据结构：vector(数组)、list(双向循环链表)
*/
#endif
...
绑定器 + 二元函数对象 =》一元函数对象
bind1st：把二元函数对象operator()(a,b)第一个形参绑定起来
bind2nd：把二元函数对象operator()(a,b)第二个形参绑定起来
*/
int main()
{
