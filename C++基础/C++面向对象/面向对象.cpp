...
int main()
{
	...
	类的成员方法一经编译，所有的方法参数，都会加一个类类型this指针，接收调用该成员方法的对象的地址
...
public:
	...
	//赋值运算符重载
	//返回值为该类对象的引用是为了支持连续的operator=赋值；如果返回类对象的话，会产生临时对象，影响代码效率。
	SeqStack& operator=(const SeqStack& src)
	{
		...
		delete[]_pstack;
		//进行赋值
		_pstack = new int[src._size];//开辟赋值对象大小的空间
		for (int i = 0; i <= src._top; ++i)
		{
			_pstack[i] = src._pstack[i];
		}
